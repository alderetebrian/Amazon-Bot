...
class AmazonAPI:
    ...
    def run(self):
        ...
        #soup = self.soup_file()
        #categorias = self.get_categorias('https://www.amazon.es/b/261-1419186-8055963?ie=UTF8&node=14627860031&ref_=sd_allcat_urban', 'prueba')
        #print(categorias)
        categorias = self.get_categorias_principal(URL_TESTING)
        print(categorias)
        time.sleep(6)
        self.makeJson('json',categorias)
        #cargando = json.loads(categorias)


        '''
        for item in cargando:
                for categoria in item['sub_categoria']:
                    print(self.get_categorias(categoria['link'],categoria['titulo']))
    ...
    def get_categorias(self, url, padre):
        time.sleep(6)
        ...
        try:
            ...
            for link in get_links:
                if len(arrow_icon) == len(arrow_text):
                    ...
                    subcategoria = {}
                    subcategoria['titulo_padre'] = padre
